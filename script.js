function handleBooking() {
    ...
    // Pega o texto visível do select (ex: "Corte clássico — R$ 55")
    const serviceText = serviceEl.options[serviceEl.selectedIndex].text;
    const barberText  = barberEl.options[barberEl.selectedIndex].text;
...
✂️ Barbeiro: ${barberText}
📅 Data: ${dateFormatted}
🕐 Horário: ${time}

Aguardo a confirmação. Obrigado!`;

    // Número do WhatsApp (somente dígitos)
    const whatsappNumber = '5583993932632';
    const url = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`;

    // Mostra toast de sucesso e abre WhatsApp
    ...
    // Pequeno delay para o usuário ver o toast antes de abrir o WA
    setTimeout(() => window.open(url, '_blank'), 800);

    // Limpa o formulário
    ['name', 'phone', 'service', 'barber', 'date', 'time'].forEach(id => {
